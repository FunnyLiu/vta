...
import copyFiles from "../src/lib/copy-files";
...
it("copy-files-success", () => {
  ...
  return Promise.all(
    packages
      ...
      .concat([fse.remove(path.resolve(cwd, "packages/delay/LICENSE"))]),
  ).then(() =>
    copyFiles(packages, filesCopyToPackages, cwd)
      .then(({ wipe }) => {
...
it("copy-files-failed", () => {
  return fse.emptyDir(path.resolve(cwd, "packages", "test-files-copy-404")).then(() =>
    copyFiles(
      [{ cwd: path.resolve(cwd, "packages", "test-files-copy-404"), pkg: "test-files-copy-404" }],
      ["LICENSE", { src: "files/404.html", dest: "404.html" }],
      cwd,
    )
      .then(
        () =>
